...
# Temperature Dashboard — FULL patched version (v2)
# Changes in this version:
#   1) KPI deltas now use st.metric delta with proper up/down arrows;
#      unchanged shows 0.00 which renders a neutral indicator.
#   2) Added an info dropdown ("What do these columns mean?") above the
#      comparison summary under KPIs.
#   3) Chart hovers: one date at top; per ADM1, show ADM1 name once then
#      all relevant info (avg, sigma, extremes) in that block.
#      We achieve this by attaching customdata to the AVG trace and
#      skipping hover on band/extreme traces.
#   4) When frequency = Seasonal, hover date shows Season + Year (e.g., JJA 2016).
#   5) Retains: Story | Chart | Options(form) layout; Percentiles section
#      uses single-radio selector and no display options; ADM1 comparisons
#      included across charts; Latest-window mini chart removed.
#
# NOTE: This is a self-contained file for drop-in replacement.
...
# ---- CSS (bigger KPI text + subtle display box) ----
st.markdown("""
<style>
...
div.disp-box { 
  border:1px solid #e5e7eb; border-radius:12px; padding:10px 12px; background:rgba(255,255,255,0.75); 
}
</style>
...
.kpi .badge {
  ...
  border:1px solid #e5e7eb; font-size:12px; font-weight:700; color:#334155;
}
.kpi .up   { background:#dcfce7; }   /* green */
.kpi .down { background:#fee2e2; }   /* red   */
.kpi .flat { background:#e2e8f0; }   /* blue/neutral */
.kpi .help { cursor:help; color:#64748b; font-weight:700; }
</style>
""", unsafe_allow_html=True)
...
INDICATOR_LABELS = (
    "Temperature","Precipitation","Temperature Thresholds","Heatwaves","Coldwaves",
...
def _arrow_badge(delta: float) -> str:
    """Arrow-only badge, snug next to the metric."""
    if not (isinstance(delta, (int, float)) and np.isfinite(delta)):
        sym, bg, tip = "—", "#e2e8f0", "No change"
    elif abs(delta) < 1e-12:
        sym, bg, tip = "—", "#e2e8f0", "No change"
    elif delta > 0:
        sym, bg, tip = "↑", "#dcfce7", f"+{delta:.2f} °C"
    else:
        sym, bg, tip = "↓", "#fee2e2", f"{delta:.2f} °C"

    # Smaller pill, slight negative left margin to “hug” the metric
    return f"""
    <span title="{tip}" style="
        display:inline-block; line-height:1; margin-left:-6px; margin-top:-2px;
...
def render_kpi(label: str, value_text: str, delta: float,
               help_text: str = "", show_symbol: bool = True):
    # decide badge style + symbol and tooltip
    if delta is None or not (isinstance(delta, (int,float)) and np.isfinite(delta)):
        klass, sym, tip = "flat", "—", "No change"
    else:
        if abs(delta) < 1e-12:
            klass, sym, tip = "flat", "—", "No change"
        elif delta > 0:
            klass, sym, tip = "up", "↑", f"+{delta:.2f} °C"
        else:
            klass, sym, tip = "down", "↓", f"{delta:.2f} °C"
...
if debug_qp:
    show_dev_debug = st.toggle("Developer debug panel", value=False,
                               help="Show resolver paths, files, and column checks.")
else:
    show_dev_debug = False

# ===================== LAYOUT: LEFT MAP / RIGHT CONTROLS + KPI =====================
...
# ===================== ADM1 MAP LOADER (latest-only TMPA) =====================
@st.cache_data(ttl=900, show_spinner=False)
def load_latest_adm1_for_map(iso3: str, freq: str, want_debug: bool = False):
    """
    For a given ISO3 and frequency, scan all *_ADM1_data.parquet files and extract the latest TMPA value per ADM1.
    Falls back to other frequencies if none found for the requested one (when want_debug=False).
    Returns either a DataFrame, or (DataFrame, tried_info) when want_debug=True.
    """
    def _scan_one(iso3_in, freq_in):
        ...
        for f in sorted(base_dir.glob("*_ADM1_data.parquet")):
            ...
            if str(freq_in) == "Monthly":
                use_cols.append("Month")
            elif str(freq_in) == "Seasonal":
                use_cols.append("Season")
            ...
            # Keep only the latest row per file/ADM1
    ...
    # First try the requested frequency
    df, dbg = _scan_one(iso3, freq)
    tried = [(freq, dbg)]

    # If empty and not in debug mode, try other freqs as graceful fallback
...
with rc:
    ...
    if "opt_adm1_t" not in st.session_state:
        st.session_state["opt_adm1_t"] = url_city if url_city in adm1_options else "Country (all)"

    sel_adm1 = st.selectbox(
        "Select Province/City/State",
        options=adm1_options,
        index=adm1_options.index(st.session_state["opt_adm1_t"]) if st.session_state["opt_adm1_t"] in adm1_options else 0,
        key="opt_adm1_t",
    ...
    if base_tmpa.empty:
        with k1: st.metric("Latest Avg Temp (TMPA)", "—", help="Latest available average temperature at the selected frequency and area.")
        with k2: st.metric("Δ vs previous point", "—", help="Change from the immediately previous period at the same frequency.")
        with k3: st.metric("Δ vs same period LY", "—", help="Change from the same month/season last year (or previous year for annual).")
        with k4: st.metric("Mean / σ in range", "—", help="Average and standard deviation over the displayed series.")
    else:
        ...
        with k1:
            render_kpi(
                "Latest Avg Temp (TMPA)",
                (f"{latest:.2f} °C" if np.isfinite(latest) else "—"),
                delta=None,
                help_text="Latest available average temperature at the selected frequency and area.",
                show_symbol=False,   # <-- no badge here
            )
        ...
        with k2:
            render_kpi(
                ...
                delta=(delta_prev if np.isfinite(delta_prev) else None),
                help_text="Change from the immediately previous period at the same frequency.",
                show_symbol=True,    # <-- keep badge
            )
        ...
        with k3:
            render_kpi(
                ...
                delta=(delta_yoy if np.isfinite(delta_yoy) else None),
                help_text="Change from the same month/season last year (or previous year for annual).",
                show_symbol=True,    # <-- keep badge
            )
        ...
        with k4:
            render_kpi(
                ...
                delta=None,
                help_text="Average and standard deviation over the displayed series.",
                show_symbol=False,   # <-- no badge here
            )


    # add a little breathing room below KPIs
    st.markdown("<div style='height: 14px'></div>", unsafe_allow_html=True)


    # === ADM1 comparison controls + info + summary (under KPIs) ===
    try:
    ...
    sel_compare = st.multiselect(
        ...
        options=_pool, max_selections=5, key="sel_compare_t",
...
- **Latest (°C)** — average temperature at the latest period.
- **Δ prev (°C)** — change from the immediately previous period.
- **Δ YoY/YoS (°C)** — change from the same month/season last year (or previous year for annual).
- **Mean (°C)** — arithmetic mean over the displayed series.
- **σ (°C)** — typical variability; from variance column if available, else std. dev. of the series.
- **Outlier** — z-score flag on the last point vs a recent window (≥2σ).
    ...
    def _kpi_for_geo(iso3_in: str, geo_label: str, freq_str: str) -> dict:
        s = _prep_single(iso3_in, geo_label, freq_str, "TMPA", "TMPV")
        if s.empty:
            return {"ADM1": geo_label, "Last date": "—", "Latest (°C)": np.nan, "Δ prev (°C)": np.nan,
                    "Δ YoY/YoS (°C)": np.nan, "Mean (°C)": np.nan, "σ (°C)": np.nan, "Outlier": "—"}
        ...
        return {
            "ADM1": geo_label, "Last date": last_date, "Latest (°C)": round(last_val, 2),
            "Δ prev (°C)": (round(last_val - prev_val, 2) if np.isfinite(prev_val) else np.nan),
            "Δ YoY/YoS (°C)": (round(last_val - yoy_val, 2) if np.isfinite(yoy_val) else np.nan),
            "Mean (°C)": round(mean_v, 2), "σ (°C)": round(sigma_v, 2), "Outlier": outlier_flag
        }
    ...
    if geo_list:
        ...
        styler = (
            summary.style
                .format(precision=2, na_rep="—")                    # optional
...
def _season_year_str(series_ts: pd.Series) -> np.ndarray:
    """Return vector like 'JJA 2016' given a datetime64[ns] Series."""
    s = series_ts.dt.month.map(MONTH_TO_SEASON).astype(str) + " " + series_ts.dt.year.astype(str)
    return s.to_numpy(dtype=object)

# ===================== CHART RENDERER (Story | Chart | Options form) =====================

# ---- Shared legend placement (horizontal, above chart) ----
...
def render_chart_with_controls(
    ...
    comp_list = st.session_state.get("sel_compare_t", []) or []
    ...
    if not sdict:
        left, _, _ = st.columns([0.2,0.6,0.2], gap="large")
    ...
    with opts_col:
        with st.form(f"form_{chart_key}"):
            ...
            show_avg  = st.checkbox("Show average", value=True, key=f"{chart_key}_avg", help="Toggle the average series.")
            show_band = st.checkbox("Show ±1σ band", value=True, key=f"{chart_key}_band", help="Shaded band from variance if available.")
            show_ext  = st.checkbox("Show extremes", value=bool(extras), key=f"{chart_key}_ext",
                                    disabled=not extras, help="Max/Min extremes if provided for this indicator.")
    ...
    with chart_col:
        ...
        for i, (label, s) in enumerate(sdict.items()):
            ...
            # Prepare customdata for AVG trace: [adm1, avg, sigma, ex1, ex2]
            # --- Build shape-safe customdata: each column is length N ---
                # --- Build shape-safe customdata for the AVG trace ---
            N = len(s2)
            ...
            # customdata columns: [0]=ADM1, [1]=avg, [2]=sigma, [3]=ex1, [4]=ex2, [5]=season_year
            ...
            # Band (no hover)
                # Band (no hover) — use sigma_arr instead of 'sigma'
            ...
            # AVG line (single hover block per ADM1 with all info)
            if show_avg:
                # Build hovertemplate: one date at top + block
                # %{customdata[0]} = label; [1]=avg; [2]=sigma; [3]=ex1; [4]=ex2
                if freq == "Seasonal":
                    # Use customdata[5] which we filled with "SEASON YEAR"
                    header = "%{customdata[5]}"
                else:
                    xfmt = "%b %Y" if freq == "Monthly" else "%Y"
                    header = "%{x|" + xfmt + "}"

                block_lines = [hover_labels_by_trace.get('avg','Average') + ": %{customdata[1]:.2f} °C"]
                if ("var" in s2.columns):
                    block_lines.append("±1σ: %{customdata[2]:.2f} °C")
                if extras:
                    if len(extras) >= 1:
                        block_lines.append(hover_labels_by_trace.get(extras[0], extras[0]) + ": %{customdata[3]:.2f} °C")
                    if len(extras) >= 2:
                        block_lines.append(hover_labels_by_trace.get(extras[1], extras[1]) + ": %{customdata[4]:.2f} °C")
                hover_tmpl = (
                    "<b>" + header + "</b><br>" +
                    "<b>%{customdata[0]}</b><br>" +
                    "<br>".join(block_lines) +
            ...
            # Extremes lines (no hover, visual only)
        ...
        if freq == "Monthly":
            ...
            fig.update_xaxes(tickformat="%Y")  # header will show Season Year
        else:
            fig.update_xaxes(tickformat="%Y")

        # If Seasonal, rewrite the hover x header to Season Year using xhoverformat isn't enough;
        # we already show "%Y" in header, but the block includes all values. For clarity, we append season in hoverlabel via text.
        if freq == "Seasonal":
            # add invisible points with text to override header via hovertemplate substitution (simple approach suffices)
            pass
...
render_chart_with_controls(
    ...
    hover_labels_by_trace={"avg":"Avg. Temp"},
    chart_key="tmpa"
)
...
render_chart_with_controls(
    ...
    hover_labels_by_trace={"avg":"Avg. Max. Temp","TMXX":"Max. of Max. Temp","TMXN":"Min. of Max. Temp"},
    chart_key="tmxa"
)
...
render_chart_with_controls(
    ...
    hover_labels_by_trace={"avg":"Avg. Min. Temp","TMNX":"Max. of Min. Temp","TMNN":"Min. of Min. Temp"},
    chart_key="tmna"
)
...
render_chart_with_controls(
    ...
    hover_labels_by_trace={"avg":"Diurnal Range"},
    chart_key="tmdr"
)

# ===================== PERCENTILES (single-radio; no display options) =====================
st.markdown("---")
